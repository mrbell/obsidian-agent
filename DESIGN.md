...
Future tables (not in initial implementation):

```sql
-- Per-note LLM-generated summary, cached for synthesis and structure jobs
...
-- Inferred topic clusters (output of vault_structure_report job)
...
|---|---|---|
| `note_summaries` | `content_sha256` changes | Delete row immediately during scan; regenerate on next summarization job |
| `topic_clusters` | Any cluster member note changes | Mark cluster as stale; re-run clustering job on next scheduled pass |

Topic cluster staleness is tracked with a `stale` boolean or `last_clustered_at` timestamp
on the cluster record, compared against the most recent index run timestamp.
...
```
obsidian-agent index              # scan vault, update DuckDB index
...
obsidian-agent/
    ...
    obsidian_agent/
        ...
        index/
            ...
            build_index.py      # incremental indexing logic
...
- `stale_task_report` (Class A)
- Note summaries cached in index (foundation for Class B jobs)
- `vault_structure_report` (Class B)
- `idea_connection_report` (Class B)
- `project_seeder` (separate design document when ready)
